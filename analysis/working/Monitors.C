...
void Monitors::Init(TTree *tree){
    ...
    if (br == nullptr){
        ...
        isArrayTraceExist_ = false;
    }
    else{
        isArrayTraceExist_ = true;
    ...
    if (br == nullptr){
        ...
        isRDTTraceExist_ = false;
    }
    else{
        isRDTTraceExist_ = true;
...
void Monitors::LoadDetGeoAndReactionConfigFile(){
    ...
    if (haha->ReadFile(detGeoFileName.c_str()) > 0 ) {
        detGeo_ = LoadDetectorGeo(haha);
        printf("... done.\n");
        PrintDetGeo(detGeo_);
        monitorsglobals::zRange[0] = detGeo_.zMin - 50 ;
        monitorsglobals::zRange[1] = detGeo_.zMax + 50 ;

        numCol_ = detGeo_.nDet;
        numRow_ = detGeo_.mDet;
    }
    else{
        printf("... fail\n");
    }
    ...
    if (kaka->ReadFile(reactionConfigFileName.c_str()) > 0 ) {
        reactionConfig_  = LoadReactionConfig(kaka);
        printf("..... done.\n");
        PrintReactionConfig(reactionConfig_);
    }
    else{
        printf("..... fail\n");
    }
...
void Monitors::LoadReactionPars(){
    ...
    if (file.is_open()){
        ...
        monitorsglobals::alpha = 299.792458*abs(detGeo_.Bfield)*monitorsglobals::q / TMath::TwoPi()/1000.; //MeV/mm
        monitorsglobals::gamm = 1./TMath::Sqrt(1-monitorsglobals::betRel*monitorsglobals::betRel);
        monitorsglobals::G = monitorsglobals::alpha*monitorsglobals::gamm*monitorsglobals::betRel*detGeo_.detPerpDist ;
...
void Monitors::PlotTDiff(bool isGated, bool isLog){
    ...
    if (monitorsglobals::isCutFileOpen1 ) monitorsglobals::text.DrawLatex(0.15, 0.8, "with Recoil gate");
    if(monitorsglobals::isTimeGateOn)monitorsglobals::text.DrawLatex(0.15, 0.7, Form("%d < coinTime < %d", monitorsglobals::timeGate[0], monitorsglobals::timeGate[1])); 
    monitorshelpers::DrawBox(monitorsglobals::htdiff, monitorsglobals::timeGate[0], monitorsglobals::timeGate[1], kGreen, 0.0);
}
...
void Monitors::Begin(TTree *tree)
{
    ...
    if (monitorsglobals::isEZCutFileOpen ) {
        ...
        if (monitorsglobals::EZCut != nullptr ) {
    ...
    if ((startTime_ == 0 && endTime_ == 0 ) || (startTime_ == endTime_)  ) branch = FindStartEndTime(tree, "rdt_t");
    if ((startTime_ == 0 && endTime_ == 0 ) || (startTime_ == endTime_)  ) branch = FindStartEndTime(tree, "ezero_t");
    if ((startTime_ == 0 && endTime_ == 0 ) || (startTime_ == endTime_)  ) branch = FindStartEndTime(tree, "elum_t");

    monitorsglobals::runTime = (endTime_-startTime_)/1e8/60.;

    printf("================ Time Range in %s timestamp branch, total Entries : %lld \n", branch.Data(),  tree->GetEntries());
    printf("start time : %llu = %.1f sec = %.0f min\n", startTime_, startTime_/1e8, startTime_/1e8/60.);
    printf("  end time : %llu = %.1f sec = %.0f min\n",  endTime_, endTime_/1e8, endTime_/1e8/60.);
    printf("  Duration : %llu = %.1f sec = %.0f min\n",  endTime_ - startTime_, (endTime_-startTime_)/1e8, monitorsglobals::runTime);

    int timeRange[2];
    timeRange[0] = TMath::Max((int) TMath::Floor(startTime_/1e8/60.), monitorsglobals::timeRangeUser[0]);
    timeRange[1] = TMath::Min((int) TMath::Ceil(endTime_/1e8/60.)   , monitorsglobals::timeRangeUser[1]);
    ...
    for (Int_t i = 0; i < NARRAY; i++) {//array loop
        ...
        monitorsglobals::heCalVxCal[i]   = new TH2F(Form("heCalVxCal%d",i),   Form("Cal PSD E vs. X (ch=%d);X (cm);E (MeV)",i),                                        500,             -2.5,  detGeo_.detLength + 2.5, 500,    monitorsglobals::energyRange[0],    monitorsglobals::energyRange[1]);
        monitorsglobals::heCalVxCalG[i]  = new TH2F(Form("heCalVxCalG%d",i),  Form("Cal PSD E vs. X (ch=%d);X (cm);E (MeV)",i),                                        500,             -2.5,  detGeo_.detLength + 2.5, 500,    monitorsglobals::energyRange[0],    monitorsglobals::energyRange[1]);
    ...
    for( int i = 0; i < numRow_; i++){
        monitorsglobals::hecalVzRow[i] = new TH2F(Form("heCalVzRow%d", i), Form("E vs. Z (ch=%d-%d); Z (cm); E (MeV)", numCol_*i, numCol_*(i+1)-1), 500, monitorsglobals::zRange[0], monitorsglobals::zRange[1], 500, monitorsglobals::energyRange[0], monitorsglobals::energyRange[1]);
        monitorsglobals::hecalVzRowG[i] = new TH2F(Form("heCalVzRow%dG", i), Form("E vs. Z (ch=%d-%d); Z (cm); E (MeV)", numCol_*i, numCol_*(i+1)-1), 500, monitorsglobals::zRange[0], monitorsglobals::zRange[1], 500, monitorsglobals::energyRange[0], monitorsglobals::energyRange[1]);
    }
...
Bool_t Monitors::Process(Long64_t entry){
    ...
    for (Int_t detID = 0; detID < NARRAY; detID++) {
        ...
        //==================== calculate Z
        if (detGeo_.firstPos > 0 ) {
            z[detID] = detGeo_.detLength*(1.0-monitorsglobals::xcal[detID]) + detGeo_.detPos[detID%numCol_];
        }
        else{
            z[detID] = detGeo_.detLength*(monitorsglobals::xcal[detID]-1.0) + detGeo_.detPos[detID%numCol_];
        }
        ...
        monitorsglobals::heCalVxCal[detID]->Fill(monitorsglobals::xcal[detID]*detGeo_.detLength,monitorsglobals::eCal[detID]);
        monitorsglobals::heCalVz->Fill(z[detID],monitorsglobals::eCal[detID]);

        //=================== Recoil Gate
        if (isRDTExist_ && (monitorsglobals::isCutFileOpen1 || monitorsglobals::isCutFileOpen2)){
            for(int i = 0 ; i < monitorsglobals::numCut1 ; i++){
                ...
                if (coinFlag && (rdtgate1 || rdtgate2) && ezGate){ 
                    ...
                    monitorsglobals::heCalVxCalG[detID]->Fill(monitorsglobals::xcal[detID]*detGeo_.detLength,monitorsglobals::eCal[detID]);
                    monitorsglobals::heVIDG->Fill(detID, e[detID]);

                    for( int i = 0; i < numRow_; i++){
                        for(int j = 0; j < numCol_; j++){
                            int k = numCol_*i+j;
            ...
            for(Int_t detID = 0; detID < NARRAY ; detID++){
                ...
                if (monitorsglobals::thetaCM > monitorsglobals::thetaCMGate ) {
                    ...
                    monitorsglobals::hExc[detID%numCol_]->Fill(monitorsglobals::Ex);
...
void Monitors::Terminate()
{
    ...
    Isotope hRecoil(reactionConfig_.recoilHeavyA, reactionConfig_.recoilHeavyZ);
    ...
    monitorshelpers::DrawLine(monitorsglobals::hEx, Sn);
    monitorshelpers::DrawLine(monitorsglobals::hEx, Sa);
    monitorshelpers::DrawLine(monitorsglobals::hEx, 4.03, 1);
    ...
    monitorshelpers::DrawLine(monitorsglobals::hEx, Sn);
    monitorshelpers::DrawLine(monitorsglobals::hEx, Sa);
    monitorshelpers::DrawLine(monitorsglobals::hEx, 4.03, 8);
